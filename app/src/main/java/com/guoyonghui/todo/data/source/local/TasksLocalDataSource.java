...
public class TasksLocalDataSource implements TasksDataSource {
    ...
    public List<Task> loadTasks() {
        ...
        if (c != null && c.getCount() > 0) {
            while (c.moveToNext()) {
                ...
                String date = c.getString(c.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_DATE));
                boolean completed = c.getInt(c.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME_COMPLETED)) == 1;

                Task task = new Task(taskId, title, description, date, completed);
                tasks.add(task);
            }
        }
        if (c != null) {
    ...
    public Task getTask(String taskId) {
        ...
        if (c != null && c.getCount() > 0) {
            ...
            task = new Task(taskId, title, description, date, completed);
        }
        if (c != null) {
            c.close();
        }
    ...
    public void saveTask(Task task) {
        ...
        values.put(TaskEntry.COLUMN_NAME_DATE, task.getDate());
